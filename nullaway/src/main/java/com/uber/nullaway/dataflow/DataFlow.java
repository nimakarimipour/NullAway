...
public final class DataFlow {
   ...
   */
    ...
    private final LoadingCache<AnalysisParams, Analysis<?, ?, ?>> analysisCache = CacheBuilder.newBuilder().maximumSize(MAX_CACHE_SIZE).build(new CacheLoader<AnalysisParams, Analysis<?, ?, ?>>() {
    ...
    private final LoadingCache<CfgParams, ControlFlowGraph> cfgCache = CacheBuilder.newBuilder().maximumSize(MAX_CACHE_SIZE).build(new CacheLoader<CfgParams, ControlFlowGraph>() {
        ...
        public ControlFlowGraph load(CfgParams key) {
            ...
            if (codePath.getLeaf() instanceof LambdaExpressionTree) {
                ...
                MethodTree enclMethod = ASTHelpers.findEnclosingNode(codePath, MethodTree.class);
     ...
     */
    private <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> Result<A, S, T> dataflow(@Nullable() TreePath path, Context context, @Nullable() T transfer) {
     ...
     */
    <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> ControlFlowGraph getControlFlowGraph(TreePath path, Context context, T transfer) {
        return dataflow(findEnclosingMethodOrLambdaOrInitializer(path), context, transfer).getControlFlowGraph();
    }
     ...
     */
    @Nullable
    public <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> A expressionDataflow(TreePath exprPath, Context context, @Nullable() T transfer) {
     ...
     */
    public <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> S finalResult(TreePath path, Context context, T transfer) {
    ...
    @Nullable
    public <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> S resultBeforeExpr(TreePath exprPath, Context context, T transfer) {
        AnalysisResult<A, S> analysisResult = resultForExpr(exprPath, context, transfer);
     ...
     */
    @Nullable
    public <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> S resultBefore(TreePath exprPath, Context context, T transfer) {
        AnalysisResult<A, S> analysisResult = resultFor(exprPath, context, transfer);
        return analysisResult == null ? null : analysisResult.getStoreBefore(exprPath.getLeaf());
    }

    @Nullable
    <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> AnalysisResult<A, S> resultForExpr(TreePath exprPath, Context context, @Nullable() T transfer) {
    ...
    @Nullable()
    private <A extends AbstractValue<A>, S extends Store<S>, T extends ForwardTransferFunction<A, S>> AnalysisResult<A, S> resultFor(TreePath exprPath, Context context, @Nullable() T transfer) {
        final TreePath enclosingPath = NullabilityUtil.findEnclosingMethodOrLambdaOrInitializer(exprPath);
    ...
    abstract static class AnalysisParams {
        ...
        private static AnalysisParams create(@Nullable() ForwardTransferFunction<?, ?> transferFunction, ControlFlowGraph cfg) {
     ...
     */
    private interface Result<A extends AbstractValue<A>, S extends Store<S>, T extends TransferFunction<A, S>> {
